...
import ldap3
from modules.certipylib.constants import WELLKNOWN_SIDS
from modules.certipylib.kerberos import get_kerberos_type1
...
class LDAPConnection:
    ...
    def connect(self, version: ssl._SSLMethod = None) -> None:
        ...
        if version is None:
            ...
            outter.debug("Authenticating to LDAP server")
        ...
        outter.debug("Bound to %s" % ldap_server)
        ...
        outter.debug("Default path: %s" % self.default_path)
        outter.debug("Configuration path: %s" % self.configuration_path)
    ...
    def get_user(
        ...
        if user is None and silent is False:
            outter.error("Could not find user %s" % repr(username))
    ...
    def get_user_sids(self, username: str):
        ...
        try:
            ...
            outter.error("Failed to get user SIDs. Try increasing -timeout")
            return sids
    ...
    def lookup_sid(self, sid: str) -> LDAPEntry:
        ...
        if len(results) != 1:
            outter.error("Failed to lookup user with SID %s" % repr(sid))
            entry = LDAPEntry(
                **{
                    "attributes": {
                        "objectSid": sid,
