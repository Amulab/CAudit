...
def get_kerberos_principal():
    ...
    output.debug("Domain retrieved from CCache: %s" % domain)

    username = "/".join(map(lambda x: x["data"].decode(), ccache.principal.components))

    output.debug("Username retrieved from CCache: %s" % username)
...
class Target:
    ...
    def from_options(
        ...
        if len(username) == 0:
            output.error("Username is not specified")
...
class DnsResolver:
    ...
    def resolve(self, hostname: str) -> str:
        ...
        if len(self.resolver.nameservers) == 0 or self.resolver.nameservers[0] is None:
            output.debug("Trying to resolve %s locally" % repr(hostname))
        else:
            output.debug(
        ...
        if ip_addr is None:
            output.error("Failed to resolve: %s" % hostname)
            return hostname
