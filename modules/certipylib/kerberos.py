...
def get_TGS(
    ...
    if target.use_sspi:
        from modules.certipylib.sspi import get_tgt

        server_name = "%s/%s" % (service, target_name)

        outter.debug("Trying to get TGS for %s via SSPI" % repr(server_name))
        ...
        if ccache:
            ...
            if domain == "":
                ...
                outter.debug("Domain retrieved from CCache: %s" % domain)

            ccache_username = "/".join(
                map(lambda x: x["data"].decode(), ccache.principal.components)
            )

            outter.debug("Using Kerberos Cache: %s" % os.getenv("KRB5CCNAME"))
            ...
            if creds is None:
                ...
                if creds is not None:
                    TGT = creds.toTGT()
                    outter.debug("Using TGT from cache")
                else:
                    outter.debug("No valid credentials found in cache. ")
            else:
            ...
            if creds is not None:
                ...
                outter.debug("Username retrieved from CCache: %s" % ccache_username)
    ...
    while True:
        if TGT is None:
            if TGS is None:
                try:
                    ...
                    outter.debug("Got TGT for %s" % repr("%s@%s" % (username, domain)))
                    ...
                    if (
                        ...
                        if (
                            ...
                            outter.debug("Got KDC_ERR_ETYPE_NOSUPP, fallback to RC4")
        ...
        if TGS is None:
            ...
            try:
                ...
                outter.debug("Got TGS for %s" % repr("%s/%s" % (service, target_name)))
            except KerberosError as e:
