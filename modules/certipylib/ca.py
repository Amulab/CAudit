...
from modules.certipylib.certificate import create_pfx, der_to_cert, load_pfx, x509
from modules.certipylib.constants import CERTIFICATION_AUTHORITY_RIGHTS
from modules.certipylib.errors import translate_error_code
from modules.certipylib.kerberos import get_TGS
from modules.certipylib.ldap import LDAPConnection, LDAPEntry
from modules.certipylib.rpc import (
    get_dce_rpc,
    get_dce_rpc_from_string_binding,
    get_dcom_connection,
)
from modules.certipylib.security import CASecurity
...
class CA:
    ...
    def rrp_dce(self):
        ...
        for _ in range(3):
            ...
            output.error("Failed to connect to remote registry")
            return None
    ...
    def get_config(self) -> Tuple[int, int, int, CASecurity]:
        ...
        output.error("Failed to get CA configuration for %s" % repr(self.ca))
    ...
    def issue(self) -> bool:
        ...
        try:
            ...
            if "E_ACCESSDENIED" in str(e):
                output.error("Got access denied trying to issue certificate")
                return False
            raise e
        ...
        if error_code == 3:
            output.info("Successfully issued certificate")
        else:
    ...
    def deny(self) -> bool:
        ...
        if error_code == 3:
            output.info("Successfully denied certificate request")
        else:
            error_msg = translate_error_code(error_code)
            output.error(
    ...
    def get_templates(self) -> str:
        if self.ca is None:
            output.error("A CA (-ca) is required")
            return False
        ...
        try:
            ...
            if "E_ACCESSDENIED" in str(e):
                output.error("Got access denied while trying to get templates")
                return None
            raise e
    ...
    def list_templates(self):
        ...
        output.info("Enabled certificate templates on %s:" % repr(self.ca))
    ...
    def enable(self, disable: bool = False) -> bool:
        ...
        if self.template is None:
            output.error("A template (-template) is required")
            return False
    ...
    def add(self, user, right, right_type):
        ...
        try:
            ...
            if "E_ACCESSDENIED" in str(e):
                output.error("Got access denied while trying to get CA security")
                return False
            raise e
        ...
        try:
            ...
            if "E_ACCESSDENIED" in str(e):
                output.error("Got access denied while trying to add %s" % right_type)
                return False
            print(e)
            raise e
    ...
    def get_backup(self) -> bytes:
        ...
        def _read_pfx(data: bytes):
            nonlocal pfx
            output.info("Got certificate and private key")
            pfx = data
    ...
    def backup(self):
        ...
        try:
            ...
            with open(pfx_out, "wb") as f:
                f.write(pfx)
            output.info("Saved certificate and private key to %s" % repr(pfx_out))
        except Exception as e:
            output.error("Backup failed: %s" % e)
