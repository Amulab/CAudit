...
class PluginExchangeNoConfigueFilter(PluginExchangeScanBase):
    """未配置发件人过滤"""

    display = "未配置发件人过滤"
    alias = "ex_no_filter_cfg"
    ...
    def run_script(self, args) -> dict:
        ...
        for entry1 in entry_generator:
            ldap_cli1 = "CN=SenderFilterConfig,CN=Message Hygiene,CN=Transport Settings,CN=" + entry1["attributes"]['cn'] + "," + ldap_cli
            ...
            for entry2 in entry_generator2:
                ...
                if entry2["attributes"]['msExchAgentsFlags'] & int("0b0001", 2) == 0:
                    result['status'] = 1
                    instance ={}
                    instance["msExchAgentsFlags"] = entry2["attributes"]['msExchAgentsFlags']
