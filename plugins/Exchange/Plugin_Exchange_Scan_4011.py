...
class PluginExchangeNoLog(PluginExchangeScanBase):
    ...
    def run_script(self, args) -> dict:
        ...
        for entry1 in entry_generator:
            ldap_cli1 = "CN=Admin Audit Log Settings,CN=Global Settings,CN=" + entry1["attributes"][
                'cn'] + "," + ldap_cli
            ...
            for entry2 in entry_generator2:
                if not entry2["attributes"]['msExchAdminAuditLogCmdlets'] or "*" not in entry2["attributes"][
                    'msExchAdminAuditLogCmdlets']:
                    result['status'] = 1
                    instance = {}
                    instance["msExchAdminAuditLogCmdlets"] = entry2["attributes"]['msExchAdminAuditLogCmdlets']
