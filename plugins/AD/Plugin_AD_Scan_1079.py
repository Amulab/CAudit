...
class PluginADAuditDCObject(PluginADScanBase):
    """审核策略中审核对象访问未正常开启"""
    display = "审核策略中审核对象访问未正常开启"
    alias = "AuditDCObjectDisable"
    ...
    def findfile(self, line, path):
        ...
        if line == 0:
            ...
            instance["Status"] = "审核策略中审核对象访问未开启审核"
            return instance
        elif line == 1:
            result['status'] = 1
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核对象访问未开启失败审核"
            return instance
        elif line == 2:
            result['status'] = 1
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核对象访问未开启成功审核"
            return instance
        else:
            pass
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce9227-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
                                    ...
                                    if flag1 == 0:  # 整个文件读取结束，flag1仍然为0，表明高级审核策略中，审核对象访问未配置
                                        result['status'] = 1
                                        path = sdir + sharedfile.get_longname()
                                        instance = {}
                                        instance["Matching file"] = path
                                        instance["Status"] = "审核策略中审核对象访问未配置"
                                ...
                                if len(results) != 0:
                                    ...
                                    if flag2 == 0:  # 整个文件读取结束，flag2仍然为0，表明审核对象访问没有定义
                                        result['status'] = 1
                                        path = sdir + sharedfile.get_longname()
                                        instance = {}
                                        instance["Matching file"] = path
                                        instance["Status"] = "审核策略中审核对象访问没有定义"
                                        instance_list.append(instance)
                                        result['data'] = {"instance_list": instance_list}
