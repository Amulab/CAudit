...
class PluginADAuditDCLogin(PluginADScanBase):
    """审核策略中审核登录事件未正常开启"""
    display = "审核策略中审核登录事件未正常开启"
    alias = "AuditDCLoginDisable"
                ...
                for sharedfile in smb.listPath(share,
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):  #识别高级审核策略
                                results = self.parse(
                                    smb, 'SYSVOL',
                                    sdir + sharedfile.get_longname())
                                flag1 = 0 #高级审核登录策略 未配置
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce9215-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
                                    ...
                                    if flag1 == 0:  #经过以上的循环之后，flag1 仍然为0，表明高级审核策略审核登录未配置
                                        result['status'] = 1
                                        path = sdir + sharedfile.get_longname()
                                        instance = {}
                                        instance[
                                            "Matching file"] = path
                                        instance[
                                            "Status"] = "高级审核策略审核登录未配置"
                                        instance_list.append(
                                            instance)
                                        result['data'] = {"instance_list": instance_list}


                            elif sharedfile.get_longname().endswith('.' + 'inf'):  #识别普通审核策略
                                ##print(1)
                                results = self.parse(
                                    smb, 'SYSVOL',
                                    sdir + sharedfile.get_longname())
                                ...
                                if len(results) != 0:
                                    ...
                                    if flag2 == 0:  #找到文件中的内容之后，没有发现字符串 AuditLogonEvents，这就代表 普通审核登录事件 没有定义
                                        result['status'] = 1
                                        path = sdir + sharedfile.get_longname()
                                        instance = {}
                                        instance[
