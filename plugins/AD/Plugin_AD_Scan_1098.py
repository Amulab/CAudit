...
class PluginADDnsZoneUpdate(PluginADScanBase):
    """域外DNS区域配置了不安全的更新"""

    display = "域外DNS区域配置了不安全的更新"
    alias = "DnsZoneUpdate2"
    ...
    def run_script(self, args) -> dict:
        ...
        for entry in entry_generator:
            ...
            if entry["attributes"]["name"] != domain and entry["attributes"]["name"] != "RootDNSServers":
                ...
                for d in entry["attributes"]["dNSProperty"]:
                    ...
                    data_len, name_len, flag, ver, identity, datas, name = struct.unpack(
                        fmt, d)
                    data["dnsProperties"].append(
                        DnsProperty(identity, datas[:data_len]))
