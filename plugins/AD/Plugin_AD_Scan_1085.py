...
class PluginADAuditDCAccountMan(PluginADScanBase):
    """审核策略中审核帐户管理未正常开启"""
    display = "审核策略中审核帐户管理未正常开启"
    alias = "AuditDCAccountManage"
    ...
    def findfile(self, line, path):
        ...
        if line == 0:
            result['status'] = 1
            instance = {"Matching file": path, "Status": "审核策略中审核帐户管理未开启审核"}
            return instance
        elif line == 1:
            result['status'] = 1
            instance = {"Matching file": path, "Status": "审核策略中审核帐户管理未开启失败审核"}
            return instance

        elif line == 2:
            result['status'] = 1
            instance = {"Matching file": path, "Status": "审核策略中审核帐户管理未开启成功审核"}
            return instance
        else:
            pass
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce9235-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
                                ...
                                if flag1 == 0:
                                    ...
                                    instance["Status"] = "审核策略中审核帐户管理未配置"
