...
class PluginADNoLDAPSign(PluginADScanBase):
    """未开启LDAP签名"""
    display = "未开启LDAP签名"
    alias = "LDAPSigningDisabled"
                    ...
                    if sharedfile.is_directory():
                        ...
                        if sharedfile.get_longname().endswith('.' + extension):
                            ...
                            if len(results) == 0:
                                continue
                            for line in results:
                                if "MACHINE\System\CurrentControlSet\Services\LDAP\LDAPClientIntegrity=4," in line:
                                    LDAPClientIntegrity = int(line.split(",")[-1].strip().replace("\r", ""))
                                    if LDAPClientIntegrity == 0:
                                        result['status'] = 1
                                        instance = {}
