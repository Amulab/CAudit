...
class PluginADNoRemoteAdminGroup(PluginADScanBase):
    """未限制本地管理员组账户执行远程管理任务"""
    display = "未限制本地管理员组账户执行远程管理任务"
    alias = "NoRemoteAdminGroup"
    ...
    def find_cpasswords(self, smb, share, base_dir, extension='xml'):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "LocalAccountTokenFilterPolicy" in line:
                                            flag += 1
                                            LATFPvalue = line.split(' ')
                                            for value in LATFPvalue:
        ...
        if flag == 0:
            result['status'] = 1
            instance = {}
            instance["Status"] = "未设置LocalAccountTokenFilterPolicy值"
            instance_list.append(instance)
