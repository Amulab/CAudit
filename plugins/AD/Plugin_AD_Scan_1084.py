...
class PluginADAuditDCLoginEvent(PluginADScanBase):
    """审核策略中审核帐户登录事件未正常开启"""
    display = "审核策略中审核帐户登录事件未正常开启"
    alias = "AuditDCLoginEvent"
    ...
    def findfile(self, line, path):
        ...
        if line == 0:
            ...
            instance["Status"] = "审核策略中审核帐户登录事件未开启审核"
            return instance
        elif line == 1:
            result['status'] = 1
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核帐户登录事件未开启失败审核"
            return instance
        elif line == 2:
            result['status'] = 1
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核帐户登录事件未开启成功审核"
            return instance
        else:
            pass
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):
                                ...
                                if len(results) != 0:
                                    ...
                                    if flag1 == 0:
                                        ...
                                        instance = {}
                                        instance["Matching file"] = path
                                        instance["Status"] = "审核策略中审核帐户登录事件未配置"
