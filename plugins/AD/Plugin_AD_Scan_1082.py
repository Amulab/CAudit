...
class PluginADAuditDCPri(PluginADScanBase):
    """审核策略中审核特权使用未正常开启"""
    display = "审核策略中审核特权使用未正常开启"
    alias = "AuditDCPrivilege"
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce9228-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
