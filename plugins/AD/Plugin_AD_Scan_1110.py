...
from modules.adi_lib.ldap.acls import SecurityDescriptor, ACCESS_MASK, ACE, has_extended_right, \
                ...
                for ace_obj in sd.dacl.aces:
                    ...
                    if ace_obj.ace.AceType == 0x05:
                        if mask.has_priv(ACCESS_MASK.ADS_RIGHT_DS_CONTROL_ACCESS):
                            if has_extended_right(ace_obj, EXTRIGHTS_GUID_MAPPING['UserForceChangePassword']):
                                instance = {}
                                instance["用户名"] = entry['attributes']["name"]
                                instance["被授予权限用户"] = sid
                                instance["权限"] = 'UserForceChangePassword'
                                instance["继承"] = is_inherited
                                instance_list.append(instance)
