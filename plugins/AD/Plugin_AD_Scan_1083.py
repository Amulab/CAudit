...
class PluginADAuditDCEvent(PluginADScanBase):
    """审核策略中审核系统事件未正常开启"""

    display = "审核策略中审核系统事件未正常开启"
    alias = "AuditDCEvent"
    ...
    def findfile(self, line, path):
        ...
        if line == 0:
            ...
            instance["Status"] = "审核策略中审核系统事件未开启审核"
            return instance
            ...
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核系统事件未开启失败审核"
            return instance
            # instance_list.append(
            #     instance)
        elif line == 2:
            ...
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核系统事件未开启成功审核"
            return instance
            # instance_list.append(
            #     instance)
        else:
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce9211-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
                                    ...
                                    if flag1 == 0:
                                        ...
                                        instance = {}
                                        instance["Matching file"] = path
                                        instance["Status"] = "审核策略中审核系统事件未配置"
