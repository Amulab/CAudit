...
class PluginADAuditDCDis(PluginADScanBase):
    """审核策略中审核策略更改未正常开启"""
    display = "审核策略中审核策略更改未正常开启"
    alias = "AuditDCDisabled"
    ...
    def findfile(self, line, path):
        ...
        if line == 0:
            ...
            instance["Status"] = "审核策略中审核策略更改未开启审核"
            return instance
            ...
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核策略更改未开启失败审核"
            return instance
            # instance_list.append(
            #     instance)
        elif line == 2:
            ...
            instance = {}
            instance["Matching file"] = path
            instance["Status"] = "审核策略中审核策略更改未开启成功审核"
            return instance
            # instance_list.append(
            #     instance)
        else:
                ...
                for sharedfile in smb.listPath(share, sdir + '*', password=None):
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + 'csv'):  # 高级审核策略
                                results = self.parse(smb, 'SYSVOL', sdir + sharedfile.get_longname())
                                flag1 = 0  # 高级审核策略更改未配置
                                if len(results) != 0:
                                    for line in results:
                                        if "0cce922f-69ae-11d9-bed3-505054503030" in line:
                                            flag1 = 1
                                ...
                                if flag1 == 0:
                                    ...
                                    instance = {}
                                    instance["Matching file"] = path
                                    instance["Status"] = "审核策略中审核策略更改未配置"
                                    instance_list.append(instance)
                                    result['data'] = {"instance_list": instance_list}

                            elif sharedfile.get_longname().endswith('.' + 'inf'):  # 普通审核策略
                                results = self.parse(smb, 'SYSVOL', sdir + sharedfile.get_longname())
                                flag2 = 0  # 普通审核策略更改未定义
                                if len(results) != 0:
                                    for line in results:
                                        if "AuditPolicyChange" in line:
                                            ...
                                            result['data'] = {"intance_list": instance_list}
                                    if flag2 == 0:
                                        result['status'] = 1
                                        path = sdir + sharedfile.get_longname()
                                        instance = {}
