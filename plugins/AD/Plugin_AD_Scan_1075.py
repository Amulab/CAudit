...
class PluginADExistAccessPrivilege(PluginADScanBase):
    """存在特权访问凭据管理器"""
    display = "存在特权访问凭据管理器"
    alias = "TrustedCredManAccessPrivilege"
                    ...
                    if sharedfile.get_longname() not in ['.', '..']:
                        if sharedfile.is_directory():
                            ...
                            if sharedfile.get_longname().endswith('.' + extension):
                                ...
                                if len(results) != 0:
                                    for line in results:
                                        if "SeTrustedCredManAccessPrivilege" in line:
                                            SeTrustedCredManAccessPrivilege = line.replace(
                                                "SeTrustedCredManAccessPrivilege =", "")
                                            if SeTrustedCredManAccessPrivilege != '':
                                                result['status'] = 1
                                                instance = {}
